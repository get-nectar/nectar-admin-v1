...
import type { LinkType, LinkStatus, LinkSurface, Platform, CurrencyCode } from '@prisma/client';

export function useCheckLink<TError = DefaultError>(args: { operation: PolicyCrudKind; where?: { id?: string; workspaceId?: string; journeyId?: string; name?: string; type?: LinkType; status?: LinkStatus; surface?: LinkSurface; channel?: Platform; messageId?: string; blastId?: string; originalUrl?: string; url?: string; shortcode?: string; shortioLinkId?: string; totalClicks?: number; uniqueClicks?: number; totalCheckouts?: number; revenueCurrency?: CurrencyCode; totalCartAdds?: number; totalCheckoutStarts?: number; groupedJourneyLinkId?: string; workspaceUserId?: string }; }, options?: (Omit<UseQueryOptions<boolean, TError, boolean>, 'queryKey'> & ExtraQueryOptions)) {
    const { endpoint, fetch } = getHooksContext();
