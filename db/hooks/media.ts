...
import type { ContentType, MediaStatus } from '@prisma/client';

export function useCheckMedia<TError = DefaultError>(args: { operation: PolicyCrudKind; where?: { id?: string; contentId?: string; commentId?: string; platformMediaId?: string; sourceUri?: string; type?: ContentType; thumbnailUri?: string; width?: number; height?: number; containerId?: string; status?: MediaStatus; index?: number; uploadOffset?: number }; }, options?: (Omit<UseQueryOptions<boolean, TError, boolean>, 'queryKey'> & ExtraQueryOptions)) {
    const { endpoint, fetch } = getHooksContext();
