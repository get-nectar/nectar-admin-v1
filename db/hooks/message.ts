...
import type { MessageType, Sentiment, MessageStatus } from '@prisma/client';

export function useCheckMessage<TError = DefaultError>(args: { operation: PolicyCrudKind; where?: { id?: string; conversationId?: string; originContentId?: string; platformMessageId?: string; content?: string; messageType?: MessageType; isFromOwner?: boolean; isReadByOwner?: boolean; detectedSentiment?: Sentiment; responseSuggestion?: string; isSentByAI?: boolean; llmAnalysisMetadataId?: string; parentCampaignId?: string; status?: MessageStatus; journeyResponseId?: string; parentJourneyId?: string; parentActionBlockId?: string; workspaceSenderId?: string; isMarkedAsSpam?: boolean; moderatedItemId?: string; quickReplies?: string; messageGroupId?: string; replyCommentId?: string; workspaceId?: string }; }, options?: (Omit<UseQueryOptions<boolean, TError, boolean>, 'queryKey'> & ExtraQueryOptions)) {
    const { endpoint, fetch } = getHooksContext();
